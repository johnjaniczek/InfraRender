...
class MixedSpectra(object):
    def __init__(self, ascii_spectra=None, meta_csv=None):
        ...
        if ascii_spectra is not None and meta_csv is not None:
            self.spectra = np.loadtxt(ascii_spectra)
            self.bands = self.spectra[:, 0]
            ...
            self.abundances = meta.iloc[:, 2:].values
            self.presence = np.zeros(self.abundances.shape)
            self.presence[np.where(self.abundances > 0)] = 1
    ...
    def single(self, index):
        ...
        sub.abundances = self.abundances[index, :]
    ...
    def append(self, new_mixture):
        ...
        if self.abundances is not None:
            self.abundances = np.vstack((self.abundances, new_mixture.proportions))
        else:
            self.abundances = new_mixture.proportions
